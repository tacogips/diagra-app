...
4. In window B, drag a marquee over empty canvas while window A watches.
...
shape while B drags it. While B drags a marquee, window A shows the growing
dashed rectangle in B's colour, and it disappears the moment B releases
(`presence-geometry.test.ts` covers both projections and the tag cap of 4
per peer headlessly).
